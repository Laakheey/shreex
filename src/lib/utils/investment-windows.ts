...
export function getMonthlyWithdrawalWindow(
  ...
  return { isOpen: isWithdrawalDay, nextWindowDate, daysUntilWindow, message };
}

export function getFixedWithdrawalWindow(
  startDate: string | Date
): WithdrawalWindow {
  const start = startOfDay(new Date(startDate));
  const today = startOfDay(new Date());
...
export function getWithdrawalWindow(
  planType: "monthly" | "fixed",
  startDate: string | Date
): WithdrawalWindow {
  return planType === "monthly"
    ? getMonthlyWithdrawalWindow(startDate)
    : getFixedWithdrawalWindow(startDate);
}
...
export function getDaysSinceStart(startDate: string | Date): number {
  return differenceInDays(startOfDay(new Date()), startOfDay(new Date(startDate)));
}
